...
@property (nonatomic, weak) HSMiddleView *middleView;
...
- (void)setUpInit {
    ...
    // 1. 设置样式, 去除tabbar上面的黑线
    self.barStyle = UIBarStyleBlack;
    
    // 2. 设置tabbar 背景图片
    
    NSBundle *currentBundle = [NSBundle bundleForClass:[self class]];
    NSString *imagePath = [currentBundle pathForResource:@"tabbar_bg@2x.png" ofType:nil inDirectory:@"FunkyFMMain.bundle"];
...
#pragma mark - 懒加载
//中间控件
- (HSMiddleView *)middleView {
    
    if (_middleView == nil) {
        HSMiddleView *middleView = [HSMiddleView shareInstance];
